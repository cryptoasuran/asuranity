
export class SHA256 {
  constructor() {
    this.K = [
      0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x3956c25b, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
      0xd807aa98, 0x12835b01, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
      0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x240ca1cc, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
      0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0xd5a79147, 0x06ca6351, 0x14292967,
      0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x53380d13, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
      0xa2bfe8a1, 0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xd6990624, 0xf40e3585, 0x106aa070,
      0x19a4c116, 0x1e376c08, 0x2748774c, 0x34b0bcb5, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f, 0x682e6ff3,
      0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
    ];
  }

  hash(message) {
    const blocks = this.preprocess(message);
    let H = [0x6a09e667, 0xbb67ae85, 0x3c6ef372, 0xa54ff53a, 0x510e527f, 0x9b05688c, 0x1f83d9ab, 0x5be0cd19];

    for (let i = 0; i < blocks.length; i += 16) {
      const W = new Array(64);
      for (let t = 0; t < 16; t++) W[t] = blocks[i + t];
      for (let t = 16; t < 64; t++) {
        W[t] = this.add(this.add(this.add(this.sigma1(W[t-2]), W[t-7]), this.sigma0(W[t-15])), W[t-16]);
      }

      let [a, b, c, d, e, f, g, h] = H;

      for (let t = 0; t < 64; t++) {
        const T1 = this.add(this.add(this.add(this.add(h, this.Sigma1(e)), this.Ch(e, f, g)), this.K[t]), W[t]);
        const T2 = this.add(this.Sigma0(a), this.Maj(a, b, c));
        h = g; g = f; f = e; e = this.add(d, T1);
        d = c; c = b; b = a; a = this.add(T1, T2);
      }

      H = [this.add(H[0], a), this.add(H[1], b), this.add(H[2], c), this.add(H[3], d),
           this.add(H[4], e), this.add(H[5], f), this.add(H[6], g), this.add(H[7], h)];
    }

    return H.map(h => h.toString(16).padStart(8, '0')).join('');
  }

  preprocess(message) {
    const bytes = new TextEncoder().encode(message);
    const bitLength = bytes.length * 8;
    const paddingLength = (448 - (bitLength + 1) % 512 + 512) % 512;
    const totalLength = bitLength + 1 + paddingLength + 64;
    const buffer = new ArrayBuffer(totalLength / 8);
    const view = new DataView(buffer);

    for (let i = 0; i < bytes.length; i++) view.setUint8(i, bytes[i]);
    view.setUint8(bytes.length, 0x80);

    const lengthOffset = totalLength / 8 - 8;
    view.setUint32(lengthOffset, Math.floor(bitLength / 0x100000000), false);
    view.setUint32(lengthOffset + 4, bitLength & 0xffffffff, false);

    const blocks = [];
    for (let i = 0; i < totalLength / 32; i++) {
      blocks.push(view.getUint32(i * 4, false));
    }
    return blocks;
  }

  Ch(x, y, z) { return (x & y) ^ (~x & z); }
  Maj(x, y, z) { return (x & y) ^ (x & z) ^ (y & z); }
  Sigma0(x) { return this.rotr(x, 2) ^ this.rotr(x, 13) ^ this.rotr(x, 22); }
  Sigma1(x) { return this.rotr(x, 6) ^ this.rotr(x, 11) ^ this.rotr(x, 25); }
  sigma0(x) { return this.rotr(x, 7) ^ this.rotr(x, 18) ^ (x >>> 3); }
  sigma1(x) { return this.rotr(x, 17) ^ this.rotr(x, 19) ^ (x >>> 10); }
  rotr(x, n) { return (x >>> n) | (x << (32 - n)); }
  add(x, y) { return (x + y) & 0xffffffff; }
}

export default SHA256;
